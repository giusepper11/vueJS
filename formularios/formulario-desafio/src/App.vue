<template>
    <div id="app">
        <h1>Formulário Desafio</h1>
        <div class="conteudo">
            <form class="painel" v-if="!enviado">
                <div class="cabecalho">Formulário</div>
                <!-- Exercicio 01 -->
                <!-- Criar uma formulário de registro -->
                <!-- Nome completo (Nome e Sobrenome) -->
                <Rotulo :nome="field.name" v-for="(field, ix) in fields" :key="ix">

                    <template v-if="field.type === 'option'">
                        <select name="" id="" v-model="formData[field.variable]">
                            <option :value="op.value" :key="op.value" v-for="op in field.option">{{op.label}}</option>
                        </select>
                    </template>

                    <input v-else :type="field.type" v-model="formData[field.variable]">

                </Rotulo>


                <!-- Exercicio 02 -->
                <!-- Só mostrar o fomulário de não tiver sido submetido -->
                <!-- Mostrar a área de Resultado apenas quando o formulário for submetido -->
                <button @click.prevent="enviar">Enviar</button>

                <!-- Exercicio 03 -->
                <!-- Crie um componente personalizado NomeCompleto -->
                <!-- Esse componente deve receber Nome e Sobrenome -->
            </form>
            <div class="painel" v-else>
                <div class="cabecalho">Resultado</div>
                <Rotulo nome="Nome">
                    <span>{{formData.nome}}</span>
                </Rotulo>
                <Rotulo nome="Sobrenome">
                    <span>{{formData.sobrenome}}</span>
                </Rotulo>
                <Rotulo nome="E-mail">
                    <span>{{formData.email}}</span>
                </Rotulo>
                <Rotulo nome="Senha">
                    <span>{{formData.senha}}</span>
                </Rotulo>
                <Rotulo nome="Armazenar Dados?">
                    <span>{{formData.saveData}}</span>
                </Rotulo>
            </div>
        </div>
    </div>
</template>

<script>
    import Rotulo from './components/Rotulo.vue'
    import NomeCompleto from "./components/NomeCompleto";

    export default {
        name: 'app',
        components: {Rotulo, NomeCompleto},
        data() {
            return {
                enviado: false,
                formData: {
                    nome: '',
                    sobrenome: '',
                    email: '',
                    senha: '',
                    saveData: true,
                },
                fields: [
                    {name: 'Nome', type: 'text', variable: 'nome'},
                    {name: 'Sobrenome', type: 'text', variable: 'sobrenome'},
                    {name: 'E-mail', type: 'text', variable: 'email'},
                    {name: 'Senha', type: 'password', variable: 'senha'},
                    {
                        name: 'Armazenar dados', type: 'option', variable: 'saveData', option: [
                            {label: "Sim", value: true},
                            {label: "Não", value: false},
                        ]
                    },

                ]
            }
        },
        methods: {
            enviar() {
                this.enviado = true
            }
        }
    }
</script>

<style>

    body {
        background-color: #ECECEC;
    }

    #app {
        font-family: 'Avenir', Helvetica, Arial, sans-serif;
        -webkit-font-smoothing: antialiased;
        -moz-osx-font-smoothing: grayscale;
        text-align: center;
        color: #2c3e50;

        display: flex;
        flex-direction: column;
    }

    .conteudo {
        display: flex;
    }

    .painel {
        flex: 1;
        background: #FFF;
        margin: 0px 10px;
        padding: 20px;
        border: 1px solid #AAA;
        border-radius: 5px;
    }

    .painel .cabecalho {
        width: 100%;
        background-color: #DDD;
        padding: 10px 0px;
        border-radius: 5px;
        font-size: 1.4rem;
    }

    #app form button {
        float: right;
        margin: 10px 0px;
        padding: 10px 20px;
        font-size: 1.4rem;
        border-radius: 5px;
        color: #FFF;
        background-color: #2196F3;
    }

    #app h1 {
        font-weight: 200;
        margin: 20px;
        padding: 0;
    }

    .mr-4 {
        margin-right: 40px;
    }
</style>
